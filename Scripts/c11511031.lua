...
function c11511031.initial_effect(c)
	--xyz summon
	aux.AddXyzProcedure(c,aux.FilterBoolFunction(Card.IsSetCard,0xff),7,2)
	c:EnableReviveLimit()
	--indes
...
function c11511031.indtg(e,c)
	return c:IsSetCard(0xff) and c~=e:GetHandler()
end
function c11511031.repfilter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsSetCard(0xff)
end
