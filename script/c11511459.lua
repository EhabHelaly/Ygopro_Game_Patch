...
function c11511459.initial_effect(c)
	-- sp
	...
	e1:SetTarget(c11511459.tg1)
	e1:SetOperation(c11511459.op1)
	c:RegisterEffect(e1)
end
function c11511459.filterF(c,e,tp,tc)
	local min=1
	if tc then min=2 end
	return c:IsSetCard(0xffb) and c:IsAbleToHand() and c:IsType(TYPE_MONSTER) 
		and Duel.IsExistingMatchingCard(c11511459.filterH,tp,LOCATION_HAND,0,min,nil,e,tp,c,tc)
end
function c11511459.filterH(c,e,tp,rc,tc)
...
function c11511459.tg1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chkc then return false end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1 
		and Duel.IsExistingTarget(c11511459.filterF,tp,LOCATION_MZONE,0,1,nil,e,tp,nil) end
...
function c11511459.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	g=Duel.SelectMatchingCard(tp,c11511459.filterH,tp,LOCATION_HAND,0,g:GetCount(),g:GetCount(),nil,e,tp,tc1,tc2)
