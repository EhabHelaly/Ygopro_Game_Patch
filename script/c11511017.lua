...
function c11511017.spfilter(c)
	return c:IsFaceup() and c:IsSetCard(0xfff) and c:GetCode()~=11511017
end
...
function c11511017.filterM1(c,tp)
	return c:IsFaceup() and c:IsSetCard(0xfff) and c:GetLevel()>0 and Duel.IsExistingTarget(c11511017.filterM2,tp,LOCATION_MZONE,0,1,c)
end
...
function c11511017.targetM(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c11511017.filterM1,tp,LOCATION_MZONE,0,1,nil,tp)  end
    	 ...
    	 e:GetHandler():RegisterFlagEffect(11511017,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
 end
