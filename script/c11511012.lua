...
function c11511012.filter(c,e,tp)
	return c:IsSetCard(0xfff) and not c:IsCode(11511012) and c:GetLevel()<5 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function c11511012.filterM1(c,tp)
	return c:IsFaceup() and c:IsSetCard(0xfff) and c:GetLevel()>0 and Duel.IsExistingTarget(c11511012.filterM2,tp,LOCATION_MZONE,0,1,c)
end
...
function c11511012.targetM(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c11511012.filterM1,tp,LOCATION_MZONE,0,1,nil,tp)  end
    	 ...
    	 e:GetHandler():RegisterFlagEffect(11511012,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
 end
