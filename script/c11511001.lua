...
function c11511001.filterH(c)
	return c:IsSetCard(0xfff) and c:IsType(TYPE_MONSTER) and not c:IsCode(11511001) and c:IsAbleToHand()
end
function c11511001.filterG(c)
	return c:IsSetCard(0xfff) and c:IsType(TYPE_MONSTER) and not c:IsCode(11511001) and c:IsAbleToGrave()
end
...
function c11511001.operation(e,tp,eg,ep,ev,re,r,rp)
    ...
    if dg:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(11511001,3)) then
...
function c11511001.targetM(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c11511001.filterM1,tp,LOCATION_MZONE,0,1,nil,tp)  end
    	 ...
    	 e:GetHandler():RegisterFlagEffect(11511001,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
 end
