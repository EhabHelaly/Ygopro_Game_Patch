...
function c11511006.cfilter(c,s)
...
function c11511006.condition(e,tp,eg,ep,ev,re,r,rp)
	...
	for seq=0,4 do
        if Duel.CheckLocation(tp,LOCATION_MZONE,seq) or not e:GetHandler():IsFaceup() then x="00" else
            local g=Duel.GetMatchingGroup(c11511006.cfilter,tp,LOCATION_MZONE,0,nil,seq)
...
function c11511006.filterH(c,e,tp)
	return c:IsSetCard(0xff) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and not c:IsCode(11511006)
end
...
function c11511006.filterM1(c,tp)
	return c:IsFaceup() and c:IsSetCard(0xff) and c:GetLevel()>0 and Duel.IsExistingTarget(c11511006.filterM2,tp,LOCATION_MZONE,0,1,c)
end
...
function c11511006.targetM(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c11511006.filterM1,tp,LOCATION_MZONE,0,1,nil,tp)  end
    	 ...
    	 e:GetHandler():RegisterFlagEffect(11511006,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
 end
